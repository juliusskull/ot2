...
function set_accion_pasos($accion){
    ...
    switch($accion){
        ...
        try {
            ...
            $obj=$new_obj;
            $obj->commit();
             $output = array('status' => true, 'massage' =>"Ok","id"=>$obj->_id );
        ...
        break;
            ...
            if($obj->delete("_id", $id)){
                $output = array('status' => true, 'massage' =>"Se borro correctamente" );
            }else{
                $output = array('status' => false, 'massage' =>"Error al borrar" , 'error' =>"");
            }
            echo json_encode($output);
            break;
        case "all":
   ...
   $obj = new Pasos();
   $obj->id_paso=0;/*0;*///$_POST["id_paso"];
   $obj->desc_campo=$_POST["desc_campo"];
   $obj->tipo=$_POST["tipo"];
   $obj->foto=$_POST["foto"];

   $obligatorio=0;
   if(strtoupper($_POST["obligatorio"])=='SI'){
        $obligatorio=1;
   }
   $obj->obligatorio=$obligatorio;
