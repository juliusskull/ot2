...
function set_accion_ot_get_id($id){
    if(file_exists("../cache/ot$id.html")){
        return file_get_contents("../cache/ot$id.html");
    }else{
        /*lo crea y me lo pasa*/
        return set_accion_ot_crear_id($id);
    }
}
function set_accion_ot_crear_id($id){
    ...
    if(file_exists("../cache/ot$id.html")){
        unlink("../cache/ot$id.html");
    }
    $myfile = fopen("../cache/ot$id.html", "w");

    $txt= $template->render(array("CAMPOS" => $array[0],"cod_empleado_asig"=>'0',"nombre_empleado_asig"=>"USUARIO", "EMPLEADOS"=>$array_empleados, "PASOS"=>$array_pasos,"MAX_ID"=>$pasos->get_max_id()));
    fwrite($myfile, $txt);
...
function set_accion_ot($accion){
    ...
    switch($accion){
        ...
        try {
            ...
            $obj=$new_obj;
            $last_id=$obj->commit();
            set_accion_ot_crear_id($last_id);
        ...
        */
        case "id":
            ...
            $template = $twig->loadTemplate("creador_p2.html");
            $array    = $obj->getOne($id);
            $op= new Operario();
            $array_empleados = $op->getAll();
        ...
        case "modificar_cavecera":

            $obj = new Ot();
            $obj->nro_ot=$_GET['nro_ot'];

            $obj->cod_empleado_asig=$_GET['cod_empleado_asig'];
            $obj->nombre_empleado_asig=$_GET['nombre_empleado_asig'];
            if(isset($_GET['observacion'])){
                $obj->observacion=$_GET['observacion'];
            }
            if(isset($_GET['motivo'])){
                $obj->motivo=$_GET['motivo'];
            }
            $obj->update_cavecera($obj);


            $output = array('status' => true, 'massage' =>"Se modifico la ot correctamente" );
            echo json_encode($output);
               break;
