          ...
          from sincronizar s where usuario   is not null  group by s.usuario";
            ...
            */
            function get_ultima_ubicacion(){
                /*
                $sql="select
                    o.nombre as usuario,(select lat from sincronizar where
                        MONTH(fchalta) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH)
                        and id=(select max(id) from sincronizar where usuario =o.nombre)) lat
                    ,(select lng from sincronizar where
                        MONTH(fchalta) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH)
                        and id=(select max(id) from sincronizar where usuario =o.nombre)) lng
                        ,(select fchalta from sincronizar where
                        MONTH(fchalta) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH)
                        and id=(select max(id) from sincronizar where usuario =o.nombre)) fchalta
                    from operario o
                    ";*/
$sql="select
                    o.nombre as usuario,(select lat from sincronizar where  id=(select max(id) from sincronizar where usuario =o.nombre)) lat
,(select lng from sincronizar where  id=(select max(id) from sincronizar where usuario =o.nombre)) lng
,(select fchalta from sincronizar where  id=(select max(id) from sincronizar where usuario =o.nombre)) fchalta
...
//echo $sql."</p>";
  ...
  function get_filtrar_ubicacion($fch,$usuario){

                $s_where=" where lat<>'' and lat<>'0.0'  ";
                if(isset($usuario)){
                    $s_where.="and  usuario ='$usuario'";

                }
              if(isset($fch)){
                    /*
                  $s_where.=" and fchalta >'$fch'";

                  $fecha =$fch;// date('Y-m-j');
                  $nuevafecha = strtotime ( '+1 day' , strtotime ( $fecha ) ) ;
                  $nuevafecha = date ( 'Y-m-j' , $nuevafecha );

                  $s_where.=" and fchalta <'$nuevafecha'";
                  */
                  $s_where.=" and DATE_FORMAT( fchalta,  '%Y-%m-%d' ) =  '$fch'";
              }
                $sql="select * from sincronizar   $s_where order by  fchalta asc ";
                //echo $sql."</p>";
                $gsent = $this->db->prepare($sql);
                $gsent->execute();
