 ...
 public function update($data)
  {
    ...
    {
      $sql = "UPDATE ot SET
            nro_ot=?,id_loc=?,localidad=?,zona=?,id_bar=?,barrio=?,id_cal=?,calle=?,altura=?,id_motivo=?,motivo=?,cod_empleado_asig=?,nombre_empleado_asig=?,cod_cuadrilla_asig=?,contratista_asig=?,fchalta=?
            WHERE id = ".$this->id;

      $this->db->prepare($sql)
           ->execute(
            array(
					$data->nro_ot,$data->id_loc,$data->localidad,$data->zona,$data->id_bar,$data->barrio,$data->id_cal,$data->calle,$data->altura,$data->id_motivo,$data->motivo,$data->cod_empleado_asig,$data->nombre_empleado_asig,$data->cod_cuadrilla_asig,$data->contratista_asig,$data->fchalta
                  )
          );
    } catch (Exception $e)
    {
 ...
 public function getAll($w=null)
  {
    ...
    try
    {
    $sql = "SELECT id,nro_ot,id_loc,localidad,zona,id_bar,barrio,id_cal,calle,altura,id_motivo,motivo,cod_empleado_asig,nombre_empleado_asig,cod_cuadrilla_asig,contratista_asig,fchalta,template_titulo,'-24.7918987' as lat, '-65.429728'as lng FROM ot_view".$where ." order by id desc";
    $gsent = $this->db->prepare($sql);
    $gsent->execute();
