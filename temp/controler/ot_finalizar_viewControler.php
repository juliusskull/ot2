...
function set_accion_ot_finalizar_view($accion){
    ...
    switch($accion){
        ...
        case "all":
            ...
            echo $template->render(array("CAMPOS" => $array));
        break;
        case "descarga-materiales":
            $materiales_ot= new Materiales_ot();
            //echo $materiales_ot->getAll_json();
            $where =null;
            $ot="";
            if(isset($_GET['ot'])){
                $where=" o.ot ='".intval($_GET['ot'])."'";
                $ot=intval($_GET['ot']);
            }

            array_to_csv_download($materiales_ot->getAll($where), "materiales_usados$ot.csv", ";");
            break;
        case "descarga":
            $id=$_GET["id"];
            $array    = $obj->getOne($id);
